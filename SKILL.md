...
| `gitpilot-git_commit` | Stage files and create a commit. Required: `path`, `message`. Optional: `add` (list of files to stage). |
...
- **Unstaged diffs**: `git_diff` (with `staged=False`) is the only tool for structured unstaged working-tree diffs — diffpilot does not cover that case.
